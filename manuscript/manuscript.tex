...
baselines. The task was to forecast MAP and predict hypotension
(MAP\,$<$\,65~mmHg) over 1--15~min, with the trained baselines evaluated by
subject-level 5-fold out-of-fold cross-validation. TiRex-2 was the strongest zero-shot forecaster, leading every
...
(0.905 vs 0.903, $p=0.337$)---on par with TFT across the short-to-mid
horizons. Its horizon-matched forecast mean absolute error (MAE) was 6.20~mmHg
at 7~min, comparable to the 7~mmHg external 7-min MAE reported for a
task-trained comparator. The in-house trained baselines had lower MAE
beyond 1~min, and the stronger PatchTST baseline led on discrimination from
5~min onward. Among the
four foundation models, TiRex-2 alone can condition on the known future
drug-infusion plan; this gave a small but significant accuracy gain
concentrated where the infusion changes ($+1.15\%$ CRPS reduction, 95\% CI
$+0.84$ to $+1.47$, at 7~min in transition windows), far smaller than the
covariate dependence of the trained models. Because TiRex-2 already led the
other foundation models \emph{without} the covariate, its overall advantage
reflects base forecast quality more than covariate access. On an independent external cohort (MOVER, 1{,}827 cases;
...
while outperforming models transferred from VitalDB. Discrimination was
label-free; well-calibrated risk additionally required a small labelled
sample from the target site to fit a post-hoc recalibration map. A naive persistence
...
5~min. On the external cohort, then, the distinctive value of a forecaster
...
competitive with task-trained models for intraoperative haemodynamic
prediction---on par with TFT but behind the stronger PatchTST baseline at the
longer horizons---with an advantage over a trivial alarm that is
horizon-dependent. Conditioning on the planned intervention is associated with
the leading model's edge, though the effect is small.
\end{abstract}
