...
\begin{abstract}
\noindent
Intraoperative hypotension is common and linked to postoperative organ
injury, motivating models that forecast mean arterial pressure (MAP) and
warn of impending hypotension. State-of-the-art models are trained on
large, site-specific labelled cohorts, which constrains portability. We
asked whether general-purpose time-series foundation models, applied
zero-shot---with no task-specific training and no labels---can rival
task-trained models, and what makes such a model clinically useful. On a
development cohort of 2{,}708 VitalDB cases we benchmarked four zero-shot
foundation models (TiRex-2, Chronos-Bolt, TimesFM-2.5, Moirai-1.1-R)
against task-trained Temporal Fusion Transformer (TFT) and PatchTST
baselines evaluated by subject-level 5-fold out-of-fold cross-validation,
forecasting MAP and predicting hypotension (MAP\,$<$\,65~mmHg) over
1--15~min. TiRex-2 was the strongest zero-shot forecaster, leading every
other foundation model on hypotension discrimination from 3~min onward
(e.g.\ area under the receiver operating characteristic curve, AUROC,
$+0.007$ to $+0.008$ at 5~min; all $p<0.001$; a tie with TimesFM-2.5 at
1~min); its lead held
even without the drug covariate, indicating it reflects base forecast
quality rather than covariate access. Against the task-trained baselines,
zero-shot TiRex-2 slightly but significantly exceeded TFT at 5~min (0.929 vs
0.923, $p=0.002$) and was statistically indistinguishable from it at 7~min
(0.905 vs 0.903, $p=0.337$), i.e.\ on par with TFT across the short-to-mid
horizons (horizon-matched forecast mean absolute error, MAE, 6.20~mmHg at
7~min, comparable to the 7~mmHg external 7-min MAE reported for a task-trained
comparator, though the in-house trained baselines had lower MAE beyond 1~min),
while the stronger PatchTST baseline led from 5~min onward. Of the
four foundation models only TiRex-2 can condition on the known future
drug-infusion plan; this covariate-awareness distinguished the exemplar and
gave a small but significant accuracy gain concentrated where the infusion
changes ($+1.15\%$ CRPS reduction, 95\% CI $+0.84$ to $+1.47$, at 7~min in
transition windows), far smaller than the covariate dependence of the
trained models. On an independent external cohort (MOVER, 1{,}827 cases;
invasive arterial MAP; 2--3$\times$ higher hypotension prevalence), the
identical zero-shot model held AUROC\,$\geq$\,0.85 across horizons, its lead
among foundation models reproduced, and it matched models trained on MOVER
while outperforming models transferred from VitalDB. A naive persistence
baseline, however, was highly competitive at short horizons---at MOVER's 60~s
cadence it exceeded every learned model (foundation and task-trained) through
5~min---so on the external cohort the distinctive value of a forecaster
emerges only at the longer horizons ($\geq$10~min), where persistence
degrades. Read against this floor, a single training-free foundation model is
a competitive and portable paradigm for intraoperative haemodynamic
prediction whose advantage over a trivial alarm is horizon-dependent, and
conditioning on the planned intervention is the capability that distinguishes
the best exemplar.
\end{abstract}
...
Covariate-awareness is the capability that sets the leading exemplar apart:
of the four foundation models, only TiRex-2 can condition on the known future
drug-infusion plan, and it is also the strongest zero-shot forecaster. We
quantified how much that capability actually contributes by comparing TiRex-2
...
requirement, but not a zero-label one. After recalibration, decision-curve analysis
...
trained on it. We probed the internal representations of all six models on an
identical 4{,}000-window subsample of each cohort, taking each model's pooled
penultimate-layer activation as a per-window representation and asking what it
linearly encodes (Methods). Three analyses converge on a single, initially
counter-intuitive answer: \emph{zero-shot forecasting skill and linear
representational structure are decoupled}.
...
zero-shot, was a competitive paradigm for intraoperative MAP forecasting and
impending-hypotension prediction. On the VitalDB development cohort the
covariate-aware exemplar TiRex-2 was on par with task-trained TFT at the
short-to-mid horizons (though below the stronger PatchTST from 5~min) and led every other zero-shot
foundation model at all horizons; on the independent MOVER cohort the
identical model held AUROC\,$\geq$\,0.85 across horizons, matched models
trained on MOVER, and beat models transferred from VitalDB. The property
that distinguished the leading exemplar was covariate-awareness---the
ability to condition on the known future drug-infusion plan. We stress the
scope of that claim: only TiRex-2 among the models tested is
covariate-aware, so this is a statement about the best exemplar, not about
foundation models as a class, three-quarters of which (the univariate
comparators) cannot ingest a covariate at all.
...
horizons is an attractive off-the-shelf option. Unlike the HPI, our approach
uses low-resolution numeric vital signs with no arterial waveform and no
proprietary index \citep{Hatib2018,Enevoldsen2022,Vistisen2024}, and unlike
both trained comparators it needs no local training run before deployment.
...
single-centre and selected for infusion cases. Fourth, the foundation models
are pretrained on large, undisclosed time-series corpora, and we cannot rule
out that public physiological datasets---including VitalDB itself---were seen
during pretraining; while the zero-shot protocol uses no task labels and no
fine-tuning, and the external MOVER cohort (a distinct, access-controlled
source) reproduces the ranking, a fully contamination-free evaluation would
require held-out data postdating each model's pretraining cut-off. Fifth,
...
plot-digitization tool \citep{WebPlotDigitizer}.
