...
DEST="${PROJECT_ROOT}/datasets/vitaldb/data/vital_files"
...
if [ ! -s "${PROJECT_ROOT}/datasets/vitaldb/data/clinical_data.csv" ]; then
  echo "[dl] fetching clinical_data.csv from ${URL_BASE}/cases ..."
  curl -fsSL --retry 4 -o "${PROJECT_ROOT}/datasets/vitaldb/data/clinical_data.csv" "${URL_BASE}/cases" \
...
{ tail -n +2 "${PROJECT_ROOT}/datasets/vitaldb/cohort_manifest.csv" | cut -d, -f1
  cat "${PROJECT_ROOT}/datasets/vitaldb/pressor_cases_phen.txt" 2>/dev/null || true
